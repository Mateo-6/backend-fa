...
import { AmiDetectedField, ParseIntentResponse } from '../dto/finance/parse-intent.dto';
import { AmiParseLogModel } from '../../infra/database/models/ami-parse-log.model';

// ---------------------------------------------------------------------------
// OpenAI JSON Schema for structured output (strict mode — all fields required,
// all nullable — optional fields are not allowed in strict mode)
// ---------------------------------------------------------------------------
const AMI_JSON_SCHEMA = {
  ...
  properties: {
    ...
    transactionType: { type: ['string', 'null'], enum: ['EXPENSE', 'INCOME', 'TRANSFER', null] },
    categoryId: { type: ['string', 'null'] },
    paymentMethodId: { type: ['string', 'null'] },
    isRecurring: { type: ['boolean', 'null'] },
    recurringFrequency: { type: ['string', 'null'], enum: ['WEEKLY', 'MONTHLY', 'YEARLY', null] },
...
- categoryId: elige el id exacto de la categoría más apropiada del listado provisto, deduciéndola del comercio o concepto aunque el usuario no la mencione explícitamente (ej. "Éxito" → supermercado/mercado, "Netflix" → entretenimiento, "gasolina" → transporte). Retorna null solo si ninguna categoría encaja.
 ...
 * using OpenAI, with Redis caching and an in-process circuit breaker.
 */
export class AmiService {
  ...
  /**
   * @param {CategoryService} categoryService Service used to load user categories.
   * @param {PaymentMethodService} paymentMethodService Service used to load user payment methods.
   * @param {RedisCacheService} redisCacheService Singleton cache client.
   */
  constructor(
   ...
   */
  async parseIntent(userId: string, text: string): Promise<ParseIntentResponse> {
    ...
    const expectedCategoryType = TRANSACTION_TYPE_TO_CATEGORY_TYPE[rawResponse.transactionType ?? 'EXPENSE'];
    const candidateCategory = rawResponse.categoryId ? categoryById.get(rawResponse.categoryId) : undefined;
    const rawCategoryId = candidateCategory && candidateCategory.type === expectedCategoryType
      ? rawResponse.categoryId! : null;
    const rawPmId = rawResponse.paymentMethodId && validPmIds.has(rawResponse.paymentMethodId)
      ? rawResponse.paymentMethodId : null;
    const rawTransferSourceId = rawResponse.transferSourceId && validPmIds.has(rawResponse.transferSourceId)
      ? rawResponse.transferSourceId : null;
    const rawTransferDestId = rawResponse.transferDestinationId && validPmIds.has(rawResponse.transferDestinationId)
      ? rawResponse.transferDestinationId : null;

    // Build detected fields
    const isTransfer = rawResponse.transactionType === 'TRANSFER';

    const categoryId: AmiDetectedField<string> = rawCategoryId && !isTransfer
      ? { value: rawCategoryId, confidence: 'high', detected: true }
      : { value: null, confidence: null, detected: false };

    const paymentMethodId: AmiDetectedField<string> = rawPmId && !isTransfer
      ? { value: rawPmId, confidence: 'high', detected: true }
      : { value: null, confidence: null, detected: false };

    const transferSourceId: AmiDetectedField<string> = rawTransferSourceId && isTransfer
      ? { value: rawTransferSourceId, confidence: 'high', detected: true }
      : { value: null, confidence: null, detected: false };

    const transferDestinationId: AmiDetectedField<string> = rawTransferDestId && isTransfer
    ...
    const result: ParseIntentResponse = {
      ...
      detectedFields: {
        ...
        type: rawResponse.transactionType
          ? { value: rawResponse.transactionType, confidence: 'high', detected: true }
          : { value: null, confidence: null, detected: false },
        categoryId,
        paymentMethodId,
        isRecurring: rawResponse.isRecurring !== null
          ? { value: rawResponse.isRecurring, confidence: 'high', detected: true }
          : { value: null, confidence: null, detected: false },
        recurringFrequency: rawResponse.recurringFrequency
          ? { value: rawResponse.recurringFrequency, confidence: 'high', detected: true }
          : { value: null, confidence: null, detected: false },
        recurringPayDay: rawResponse.recurringPayDay !== null
          ? { value: rawResponse.recurringPayDay, confidence: 'high', detected: true }
          : { value: null, confidence: null, detected: false },
        applyToBudget: rawResponse.applyToBudget !== null
          ? { value: rawResponse.applyToBudget, confidence: 'high', detected: true }
          : { value: null, confidence: null, detected: false },
  ...
  private async getCachedCategories(userId: string): Promise<Category[]> {
    const key = RedisCacheService.categoriesKey(userId);
    const cached = await this.redisCacheService.get<Category[]>(key);
    if (cached) return cached;

    const categories = await this.categoryService.findAll(userId);
    await this.redisCacheService.set(key, categories, CACHE_TTL_SECONDS);
    return categories;
  }
   ...
   */
  private async getCachedPaymentMethods(userId: string): Promise<PaymentMethod[]> {
    const key = RedisCacheService.paymentMethodsKey(userId);
    const cached = await this.redisCacheService.get<PaymentMethod[]>(key);
    if (cached) return cached;

    const paymentMethods = await this.paymentMethodService.findAllByUser(userId);
    await this.redisCacheService.set(key, paymentMethods, CACHE_TTL_SECONDS);
  ...
  private buildEmptyResponse(parseId: string): ParseIntentResponse {
    const empty: AmiDetectedField<never> = { value: null, confidence: null, detected: false };
    return {
      parseId,
      detectedFields: {
        amount: empty,
